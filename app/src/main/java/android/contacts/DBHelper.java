...
public class DBHelper extends SQLiteOpenHelper {
    ...
    private Person getLast(){
        Cursor query = getWritableDatabase().rawQuery("select * from " + TABLE +" order by id desc limit 1", null);
    ...
    public void update(Person person) {
        ...
        values.put("ship", ship);
        Log.e("log","update "+ ship + " " + person.toString());

        getWritableDatabase().update(TABLE, values, "id=?", new String[]{person.getId() + ""});
    }
